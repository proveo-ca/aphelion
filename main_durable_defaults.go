...
func installDailyReviewRecipe(cfg *config.Config, store *session.SQLiteStore, opts installDailyReviewRecipeOptions) (dailyReviewRecipeInstallResult, error) {
	...
	bootstrapLLM, err := durableChildRecipeBootstrap(recipe, cfg)
	if err != nil {
		return result, err
	}
...
func durableChildRecipeBootstrap(recipe durableChildRecipe, cfg *config.Config) (core.NodeLLMBootstrap, error) {
