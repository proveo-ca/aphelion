...
func TestRenderOperationPhaseBundlePromptIsConciseAndHidesRawLeaseDetails(t *testing.T) {
	...
	for _, want := range []string{"Approve:\nInspect approval rendering", "Review only continuation prompt rendering", "Covers:\n- phase 1: Inspect approval rendering", "phase 2: Patch approval rendering"} {
...
func TestMaterializePlanBudgetCanDiscloseEscalatedReadOnlyLane(t *testing.T) {
	...
	sender.mu.Unlock()
	if !strings.Contains(inlineText, "Approve plan:\nCheck nonsecret adapter auth status") || !strings.Contains(inlineText, "Covers:\n- Step 1: Check nonsecret adapter auth status") || !strings.Contains(inlineText, "Step 2: Patch local status reporting") {
