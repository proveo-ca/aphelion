...
func renderReflectionRequest(input *reflectionInput) string {
	...
	b.WriteString("Be selective. Most material should be ignored. Only write items that are durable, reusable, or likely to matter across future turns.\n")
	...
	b.WriteString("Use sections this way: MEMORY for stable personal/context facts, KNOWLEDGE for structured reusable facts with provenance when helpful, DECISIONS for changed commitments or durable lessons, QUESTIONS for open threads worth carrying, RHIZOME for genuine concept links rather than summaries.\n\n")
