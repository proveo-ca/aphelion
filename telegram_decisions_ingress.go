...
type telegramDecisionAcceptedRouter interface {
	RouteAccepted(ctx context.Context, msg core.InboundMessage) error
}

func (h *telegramDecisionHandler) routeDecisionMessage(ctx context.Context, msg core.InboundMessage) error {
	if h == nil || h.router == nil {
		return nil
	}
	if accepted, ok := h.router.(telegramDecisionAcceptedRouter); ok {
...
func (h *telegramDecisionHandler) routeDeferredDecisionMessage(ctx context.Context, msg core.InboundMessage, surface string, updateKind string) error {
	if h == nil || h.router == nil {
		return nil
	}
	accepted, ok := h.router.(telegramDecisionAcceptedRouter)
	if h.store == nil || !ok {
		return h.routeDecisionMessage(ctx, msg)
	}
	msg.IngressSurface = strings.TrimSpace(surface)
	msg.IngressUpdateID = telegramDecisionResumeUpdateID(msg, msg.IngressSurface)
...
func (h *telegramDecisionHandler) recordDecisionResumeAccepted(msg core.InboundMessage, updateKind string) (session.TelegramIngressTransitionResult, error) {
