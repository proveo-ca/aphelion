...
`
	...
	if cfg.Tools.WebSearch.Enabled || cfg.Tools.WebSearch.MaxCount != 10 || cfg.Tools.WebSearch.DefaultCount != 5 || cfg.Tools.WebSearch.Timeout != "15s" || cfg.Tools.WebSearch.CacheTTL != "15m" {
		t.Fatalf("tools.web_search defaults = %#v, want disabled 5/10 15s/15m", cfg.Tools.WebSearch)
	}
	if !reflect.DeepEqual(cfg.Tools.WebSearch.ProviderOrder, []string{"openai_hosted", "brave"}) || !cfg.Tools.WebSearch.OpenAIHosted.Enabled || cfg.Tools.WebSearch.OpenAIHosted.ContextSize != "medium" || cfg.Tools.WebSearch.Brave.Enabled || cfg.Tools.WebSearch.Brave.APIKeyEnv != "BRAVE_API_KEY" {
		t.Fatalf("tools.web_search provider defaults = %#v, want hosted first brave fallback defaults", cfg.Tools.WebSearch)
	}
