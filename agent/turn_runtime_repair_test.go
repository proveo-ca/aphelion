...
func TestRepairToolInputUnwrapsObjectWithEscapedNewlineValues(t *testing.T) {
	t.Parallel()

	got, err := repairToolInput(json.RawMessage(`"{\"path\":\"reports/one.txt\",\"content\":\"line one\\nline two\\n\"}"`))
	if err != nil {
		t.Fatalf("repairToolInput() err = %v", err)
	}
	var decoded struct {
	...
	if err := json.Unmarshal(got, &decoded); err != nil {
		t.Fatalf("decode repaired input %s: %v", string(got), err)
	}
	if decoded.Path != "reports/one.txt" || decoded.Content != "line one\nline two\n" {
		t.Fatalf("decoded input = %#v, want newline-preserving object", decoded)
	}
}

func TestRepairToolInputRejectsTruncatedStringWrappedObject(t *testing.T) {
	t.Parallel()

	_, err := repairToolInput(json.RawMessage(`"{\"path\":\"reports/one.txt\",\"content\":\"line one"`))
	if err == nil {
		t.Fatal("repairToolInput() err = nil, want truncated string-wrapped object rejected before tool execution")
	}
}
...
func TestRunTurnRepairsJSONStringWrappedArgumentsWithEscapedNewlinesBeforeExecution(t *testing.T) {
	t.Parallel()
	...
	provider := &mockProvider{
		complete: func(_ context.Context, call int, messages []Message, tools []ToolDef) (*Response, error) {
			...
			case 2:
				last := messages[len(messages)-1]
				if last.Role != "tool" || last.ToolName != "write_file" {
					t.Fatalf("last tool message = %#v, want write_file tool result", last)
				}
				return &Response{Content: "done"}, nil
			default:
				t.Fatalf("unexpected call %d", call)
	...
	tools := &mockTools{
		...
		exec: func(_ context.Context, name string, input json.RawMessage) (string, error) {
			...
			if err := json.Unmarshal(input, &decoded); err != nil {
				t.Fatalf("decode tool input %s: %v", string(input), err)
			}
			if decoded.Path != "reports/one.txt" || decoded.Content != "line one\nline two\n" {
				t.Fatalf("decoded tool input = %#v, want newline-preserving JSON object", decoded)
			}
			return "write_file_ok path=reports/one.txt bytes=18 append=false", nil
		},
	}
...
func TestRunTurnInjectsToolErrorForTruncatedStringWrappedToolArguments(t *testing.T) {
	t.Parallel()
	...
	tools := &mockTools{
		...
		exec: func(_ context.Context, _ string, input json.RawMessage) (string, error) {
			t.Fatalf("write_file should not execute with truncated string-wrapped arguments: %s", string(input))
			return "", nil
		},
	}
