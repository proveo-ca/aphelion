...
func (r *decisionTestRouter) Status(chatID int64) core.SessionStatus { return r.status }
...
func callbackDataForButton(t testingT, rows [][]telegram.InlineButton, label string) string {
	t.Helper()
...
func waitForDecisionInline(t testingT, sender *decisionTestSender) decisionInlineCall {
	t.Helper()
...
func waitForDecisionEdit(t testingT, sender *decisionTestSender, count int) decisionEditCall {
	t.Helper()
	deadline := time.Now().Add(time.Second)
	for time.Now().Before(deadline) {
...
func waitForStoredPendingDecision(t testingT, store *session.SQLiteStore, kind decision.Kind) session.PendingDecisionRecord {
	t.Helper()
	deadline := time.Now().Add(time.Second)
	for time.Now().Before(deadline) {
