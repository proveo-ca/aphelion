...
func ReconcileDurableAgentsForConfig(cfg *config.Config, opts durableAgentReconcileOptions, deps DurableAgentDeps) (*durableAgentReconcileResult, error) {
	return reconcileDurableAgentsForConfig(cfg, opts, deps)
}
...
func reconcileDurableAgentsForConfig(cfg *config.Config, opts durableAgentReconcileOptions, deps DurableAgentDeps) (*durableAgentReconcileResult, error) {
	...
	defaultBootstrap := core.NodeLLMBootstrap{}
	if deps.DefaultBootstrap != nil {
		defaultBootstrap = core.NormalizeNodeLLMBootstrap(deps.DefaultBootstrap(cfg))
	}
