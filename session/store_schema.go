...
const schemaVersion63 = 63
const schemaVersion64 = 64
const schemaVersion65 = 65
...
func migrateCurrentSchemaVersion(tx *sql.Tx, currentVersion int) (int, error) {
	...
	if version == schemaVersion62 {
		...
		if _, err := tx.Exec(`INSERT INTO schema_version(version) VALUES (?)`, schemaVersion63); err != nil {
			return 0, fmt.Errorf("insert schema version %d: %w", schemaVersion63, err)
		}
		version = schemaVersion63
	}
	if version == schemaVersion63 {
		if err := migrateSchemaV63ToV64(tx); err != nil {
			return 0, err
		}
		if _, err := tx.Exec(`INSERT INTO schema_version(version) VALUES (?)`, schemaVersion64); err != nil {
			return 0, fmt.Errorf("insert schema version %d: %w", schemaVersion64, err)
		}
		version = schemaVersion64
	}
	if version == schemaVersion64 {
		if err := migrateSchemaV64ToV65(tx); err != nil {
			return 0, err
		}
		if _, err := tx.Exec(`INSERT INTO schema_version(version) VALUES (?)`, schemaVersion65); err != nil {
			return 0, fmt.Errorf("insert schema version %d: %w", schemaVersion65, err)
		}
		version = schemaVersion65
	}
	if version == schemaVersion65 {
		if err := migrateSchemaV65ToV66(tx); err != nil {
			return 0, err
		}
		if _, err := tx.Exec(`INSERT INTO schema_version(version) VALUES (?)`, schemaVersion66); err != nil {
			return 0, fmt.Errorf("insert schema version %d: %w", schemaVersion66, err)
		}
		version = schemaVersion66
	}
	if version == schemaVersion66 {
		if err := migrateSchemaV66ToV67(tx); err != nil {
			return 0, err
		}
		if _, err := tx.Exec(`INSERT INTO schema_version(version) VALUES (?)`, schemaVersion67); err != nil {
			return 0, fmt.Errorf("insert schema version %d: %w", schemaVersion67, err)
		}
		version = schemaVersion67
	}
	if version == schemaVersion67 {
		if err := migrateSchemaV67ToV68(tx); err != nil {
			return 0, err
		}
		if _, err := tx.Exec(`INSERT INTO schema_version(version) VALUES (?)`, schemaVersion68); err != nil {
			return 0, fmt.Errorf("insert schema version %d: %w", schemaVersion68, err)
		}
		version = schemaVersion68
	}
