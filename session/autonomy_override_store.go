...
func (s *SQLiteStore) CreateOperatorAutonomyOverride(override OperatorAutonomyOverride) (OperatorAutonomyOverride, error) {
	...
	`, override.ID, override.AdminUserID, override.ChatID, override.Mode, override.Scope, override.Reason, override.CreatedAt.UTC().Format(time.RFC3339Nano), override.ExpiresAt.UTC().Format(time.RFC3339Nano), revokedAt, override.UpdatedAt.UTC().Format(time.RFC3339Nano)); err != nil {
...
func (s *SQLiteStore) OperatorAutonomyOverride(id string) (OperatorAutonomyOverride, bool, error) {
	...
	row := s.db.QueryRow(`
		SELECT override_id, admin_user_id, chat_id, mode, scope, reason,
			created_at, expires_at, revoked_at, updated_at
