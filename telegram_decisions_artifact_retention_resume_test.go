...
func TestResumePendingArtifactRetentionRecordsSyntheticIngressAndClearsPending(t *testing.T) {
	...
	if routed.IngressSurface != telegramArtifactRetentionDecisionResumeIngressSurface || routed.IngressUpdateID != 801 || routed.TelegramThreadID != 3 {
	...
	record, ok, err := store.TelegramIngressUpdate(telegramArtifactRetentionDecisionResumeIngressSurface, 801)
	if err != nil || !ok {
...
func TestResumePendingArtifactRetentionKeepsPendingWhenSyntheticRouteFails(t *testing.T) {
	...
	record, ok, err := store.TelegramIngressUpdate(telegramArtifactRetentionDecisionResumeIngressSurface, 802)
	if err != nil || !ok {
		t.Fatalf("TelegramIngressUpdate() ok=%t err=%v", ok, err)
	}
...
func TestRestartLoadedArtifactRetentionCallbackResumesPendingMessage(t *testing.T) {
	...
	handlerBeforeRestart := newTelegramDecisionHandler(senderBeforeRestart, &decisionTestRouter{}, brokerBeforeRestart, store)
	...
	if routed.IngressSurface != telegramArtifactRetentionDecisionResumeIngressSurface || routed.IngressUpdateID != 9101 {
	...
	if _, err := store.PendingArtifactRetention(telegramSessionOwnerKey(msg)); !errors.Is(err, sql.ErrNoRows) {
