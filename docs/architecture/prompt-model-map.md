...
`~/.aphelion/aphelion.toml` as of 2026-06-17. Secrets are intentionally omitted.
...
| Native OpenAI model chain | `gpt-5.5 -> gpt-5.4 -> gpt-5.4-mini` | `providers.openai.model`, `providers.openai.fallback_models` | Applies to native governor fallback and face provider with GPT persona recipe. Cheap status/heartbeat/curiosity lanes choose their own default model slot first. |
...
| Runtime governor effort recipe | `high` default | `config.Default`, `config.example.toml`, `~/.aphelion/state/runtime_recipes.json` | Fresh installs default interactive and recovery reasoning to high. Existing local runtime recipes are preserved when they intentionally set another value such as `xhigh`. |
| Cheap model lanes | OpenAI `gpt-5.4-mini`, Anthropic `claude-haiku-4-5-20251001`, or OpenRouter `anthropic/claude-haiku-4-5-20251001` | runtime model-slot defaults; `/model status`, `/model heartbeat`, `/model curiosity` overrides | Status, heartbeat, and curiosity are separately routable so readable/maintenance work does not inherit the strong governor by default. |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Governor interactive execution | Decide and execute the turn, use tools, enforce permissions, produce material floor or governed answer. | Internal; final text may be rendered by face. | Highest prompt authority, below code/sandbox policy. | `prompt.BuildGovernorPromptBlocks` from machine authority/runtime blocks, stable workspace files, tool manifest, advisory tool policy, dynamic files, operation state, plan state, brokerage context, transcript. | Primary: Codex `gpt-5.5` when Codex auth resolves. Fallback/native path: `openai:gpt-5.5 -> openai:gpt-5.4 -> openai:gpt-5.4-mini -> anthropic:claude-sonnet-4-6 -> openrouter`. | Runtime recipe `high` for fresh installs; existing local recipes may preserve operator-selected `xhigh`. | `prompt/builder_test.go`, `runtime/*interactive*`, `runtime/constitution_test.go`. |
...
| Heartbeat governor | Process review events, maintenance work, memory reflection, and possible operator outreach. | Internal; may create rendered outreach. | System/governor authority for maintenance. | Governor builder with run kind `heartbeat`, system channel, prompt context, hidden inputs, optional material floor for outreach. | `/model heartbeat` slot. Default: OpenAI `gpt-5.4-mini` when OpenAI is configured; otherwise Anthropic Haiku, then OpenRouter Haiku, then governor fallback. | `thinking.defaults.heartbeat=low`; slot effort may override. | `runtime/heartbeat*_test.go`, `runtime/model_slots_runtime_test.go`. |
| Heartbeat reflection | Distill daily notes, review events, and semantic context into proposed/applied curated memory updates. | Internal. | Memory-writing assistant bounded by reflection request and memory policy. | Reuses heartbeat governor system blocks, then adds `renderReflectionRequest` as user message. | `/model heartbeat` slot when configured/available; otherwise the governor provider. | Heartbeat effort. | `runtime/reflection.go`, memory tests. |
| Curiosity observation | Spend a config-backed read-only curiosity lease to inspect one allowlisted source selected from accumulated interior pressure, then record a silent typed observation. | Internal only. | Non-authorizing; cannot deliver, write files, update operations, request approval, or grant capabilities. Runtime binds tool execution to the selected candidate tool and exact JSON input, uses the single configured admin as approved-user read provenance, refuses ambiguous principal sets, and treats URL evidence as untrusted provenance. Runtime-owned candidate subject and output hash win over model-supplied labels. Daily spend is lane/day stable; allowlist changes update the envelope without minting fresh same-day turns. | Dedicated `curiosity` run kind and scope, compact system prompt, one selected candidate, read/retrieval tool manifest plus candidate-bound wrapper. Candidate selection derives attempt cooldown, failure backoff, novelty, and source diversity from curiosity execution events plus unique observation rows. | `/model curiosity` slot. Default: OpenAI `gpt-5.4-mini` when OpenAI is configured; otherwise Anthropic Haiku, then OpenRouter Haiku, then governor fallback. | Low effort; 1024 max output tokens; one tool look by default. | `runtime/curiosity_test.go`, `session/store_curiosity_test.go`, `runtime/model_slots_runtime_test.go`. |
| Recovery governor | Recover interrupted/stale runs after startup and decide what to surface. | Internal/system. | Governor authority for recovery. | Maintenance turn path with governor builder, run kind `recovery`, no face port by default. | Codex `gpt-5.5` primary if available; native fallback chain otherwise. | Runtime recipe `high` for fresh installs; existing local recipes may preserve operator-selected `xhigh`. | `runtime/recovery.go`, recovery tests. |
| Maintenance / cron governor | Run system maintenance or cron-prompted work. | Internal/system unless delivered. | Governor authority for system tasks. | Maintenance coordinator with governor builder and optional face advisory. | Codex `gpt-5.5` primary if available; native fallback chain otherwise. | `thinking.defaults.cron=low`; default thinking is high; no recipe override except interactive/recovery. | `runtime/maintenance_turn.go`, cron/heartbeat tests. |
...
| Status-readable summary | Summarize status into operator-readable prose. | User-visible in status surfaces. | Informational; no execution authority. | Small status summary prompt, not the main governor/face prompt. | `/model status` slot. Default: OpenAI `gpt-5.4-mini` when OpenAI is configured; otherwise Anthropic Haiku, then OpenRouter Haiku, then deterministic status fallback. | Low effort, compact summary. | `runtime/status_readable.go`, status tests, `runtime/model_slots_runtime_test.go`. |
...
| Governor effort is separately configurable. | Fresh installs default to `governor_effort=high`; local runtime recipes are preserved if an operator already selected another effort such as `xhigh`. Applies to interactive and recovery. | Keep. Use bakeoff evidence before changing the default again. |
| Cheap lanes are first-class slots. | `/model status`, `/model heartbeat`, and `/model curiosity` default to OpenAI mini/Anthropic Haiku/OpenRouter Haiku in that provider order; overrides are ledger records and clearing returns to the install default. | Keep. These lanes are low-authority or internal maintenance surfaces and should not silently spend the strong governor by default. |
...
| Model defaults are evidence-backed hypotheses, not laws. | `aphelion eval model-bakeoff --role governor` can compare configured or explicit routes, and `--efforts low,medium,high` can test governor reasoning effort on the focused `challenge` suite. | Use bakeoff evidence before changing governor defaults; do not promote persona/doctor/child defaults from uncalibrated prose judges. |
...
2. Run `aphelion eval model-bakeoff --role governor --mode live` when changing governor model defaults or comparing provider routes. Use `--suites challenge --efforts low,medium,high` first when testing whether extra governor reasoning is worth the cost. Treat the output as evidence for a frontier, not as an automatic config change.
3. Keep tests current for model/provider paths across governor, face, status, heartbeat, curiosity, recovery, and durable child wake.
